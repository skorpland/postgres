...
function initiate_upgrade {
    ...
    if [ "$OLD_BOOTSTRAP_USER" = "postgres" ]; then
        echo "8.2. Swap postgres & powerbase_admin roles as we're upgrading a project with postgres as bootstrap user"
    ...
    if [ "$IS_NIX_UPGRADE" = "true" ]; then
        LC_ALL=en_US.UTF-8 LC_CTYPE=$SERVER_LC_CTYPE LC_COLLATE=$SERVER_LC_COLLATE LANGUAGE=en_US.UTF-8 LANG=en_US.UTF-8 LOCALE_ARCHIVE=/usr/lib/locale/locale-archive su -c ". /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh && $PGBINNEW/initdb --encoding=$SERVER_ENCODING --lc-collate=$SERVER_LC_COLLATE --lc-ctype=$SERVER_LC_CTYPE -L $PGSHARENEW -D $PGDATANEW/ --username=powerbase_admin" -s "$SHELL" postgres
    else
        su -c "$PGBINNEW/initdb -L $PGSHARENEW -D $PGDATANEW/ --username=powerbase_admin" -s "$SHELL" postgres
    fi

    # This line avoids the need to supply the powerbase_admin password on the old
    # instance, since pg_upgrade connects to the db as powerbase_admin using unix
    # sockets, which is gated behind scram-sha-256 per pg_hba.conf.j2. The new
    # instance is unaffected.
    if ! grep -q "local all powerbase_admin trust" /etc/postgresql/pg_hba.conf; then
        echo "local all powerbase_admin trust
...
EOF
    ...
    # removing powerutils config as to allow the latest one provided by the latest image to be used
    rm -f "$MOUNT_POINT/conf/powerutils.conf" || true
